...
try:
    ...
    from effdet import init_effdet_model, inference_effdet_model
    
except ImportError:
    warnings.warn(
        'Please install MMPose for NTURGB+D pose extraction and EfficientDET.'
    )  # noqa: E501
    

# mmdet_root = ''
mmpose_root = 'C:/Users/Hidrolatina/Downloads/mmpose'

args = abc.abstractproperty()
args.obj_list = ['person']
args.det_checkpoint = 'https://github.com/EquipoVandV/EfficientDetVandV/blob/main/effdet/logs/person_coco/efficientdet-d2_58_8260_best.pth'
args.det_score_thr = 0.5
args.pose_config = f'{mmpose_root}/configs/body/2d_kpt_sview_rgb_img/topdown_heatmap/coco/hrnet_w48_coco_384x288_udp.py'  # noqa: E501
args.pose_checkpoint = 'https://download.openmmlab.com/mmpose/top_down/udp/hrnet_w48_coco_384x288_udp-0f89c63e_20210223.pth'  # noqa: E501
...
def detection_inference(frame_paths):
    model = init_effdet_model(args.det_checkpoint, args.obj_list)
    results = []
    ...
    for frame_path in frame_paths:
        ...
        out= inference_effdet_model(model, frame_img, threshold=args.det_score_thr)
        ...
        for j in range(len(out['bbox'])):
            ...
            p_det_n=[x1,y1,x2,y2,score]
            p_det.append(p_det_n)

        result=np.asarray(p_det)
...
def is_easy_example(det_results, num_person):
    ...
    if thre_bbox(det_results) == num_person:
        det_results = [x[x[..., -1] > 0.1] for x in det_results]
...
def ntu_det_postproc(vid, det_results):
    ...
    print(f'\nHard {n_person}-person Example, found {len(tracklets)} tracklet, el código no te va dejar por la precision de detección')
    if n_person == 1:
        if len(tracklets) == 1:
...
def pose_inference(args, frame_paths, det_results):
    ...
    for i, (f, d) in enumerate(zip(frame_paths, det_results)):
        # Align input format
        d = [dict(bbox=x) for x in list(d) if x[-1] > 0.1]
...
def ntu_pose_extraction(vid):
    ...
    det_results = detection_inference(frame_paths)
    logging.critical(len(det_results))
...
def parse_args():
    ...
    parser.add_argument('--det_score', type=float, default='0.6')
...
if __name__ == '__main__':
    ...
    args.det_score = global_args.det_score
