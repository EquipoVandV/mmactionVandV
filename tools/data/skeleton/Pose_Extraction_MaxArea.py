...
# mmdet_root = 'C:/Users/Hidrolatina/Downloads/mmdet'
mmpose_root = (pathlib.Path(pkg_resources.resource_filename('mmpose',''))).parent

args = abc.abstractproperty()
args.det_config = f'{mmpose_root}/demo/mmdetection_cfg/faster_rcnn_r50_fpn_coco.py'  # noqa: E501
args.det_checkpoint = 'https://download.openmmlab.com/mmdetection/v2.0/faster_rcnn/faster_rcnn_r50_fpn_1x_coco/faster_rcnn_r50_fpn_1x_coco_20200130-047c8118.pth'  # noqa: E501
...
def is_easy_example(det_results, num_person):
    threshold = args.det_score_thr #0.95
    ...
    if thre_bbox(det_results) == num_person:
        det_results = [x[x[..., -1] > args.det_score_thr] for x in det_results]
...
def ntu_det_postproc(vid, det_results):
    ...
    label = int(vid.split('/')[-1].split('A')[1][:2])
    # label= 1
...
def ntu_pose_extraction(vid):
    ...
    anno['label'] = int(osp.basename(vid).split('A')[1][:2])
    # anno['label'] = 1
...
def parse_args():
    ...
    parser.add_argument('--video', type=str, help='source video',default=os.getenv('vid'))
    parser.add_argument('--output', type=str, help='output pickle name',default=os.getenv('pkl'))
